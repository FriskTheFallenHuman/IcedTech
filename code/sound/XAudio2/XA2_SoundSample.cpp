...
bool idSoundSample_XAudio2::LoadGeneratedSample( const idStr &filename ) {
	idFileScoped fileIn( fileSystem->OpenFileReadMemory( filename ) );
	if ( fileIn != NULL ) {
		uint32_t magic;
...
// jmarshall - only re-create the idwavs if need regeneration

		// In production we only load the idWav files.
		...
		{
			// Check to see if our .wav file is out of date.
			// Load in the generated file.
			idFile *generatedFile = fileSystem->OpenFileRead(generatedName, false);
			bool shouldWriteGenerated = false;
			if (generatedFile)
			{
				// Load the source file.
				sampleName.SetFileExtension(".wav");
				idFile *sourceFile = fileSystem->OpenFileRead(sampleName, false);				
				if (sourceFile)
				{
					// Check the timestamps
					if (sourceFile->Timestamp() > generatedFile->Timestamp())
					{
						shouldWriteGenerated = true;
					}
					else
					{
						// If we should load the generated, load the generated.
						loaded = LoadGeneratedSample(generatedName);
					}
					fileSystem->CloseFile(sourceFile);
				}
				fileSystem->CloseFile(generatedFile);
			}
			...
			if (loaded && shouldWriteGenerated) {
