	...
	for (search = searchPaths; search; search = search->next) {
		if (search->dir && (searchFlags & FSFLAG_SEARCH_DIRS)) {
			...
			if (searchFlags & FSFLAG_RETURN_FILE_MEM) {
				byte *memfilebuffer = new byte[file->fileSize];
				file->Read(memfilebuffer, file->fileSize);
				idFile_Memory * memFile = new idFile_Memory(file->name, (const char *)memfilebuffer, file->fileSize);
				delete file;
				return memFile;
			}
...
idFile *idFileSystemLocal::OpenFileReadMemory(const char *relativePath, bool allowCopyFiles, const char* gamedir) {
	return OpenFileReadFlags(relativePath, FSFLAG_SEARCH_DIRS | FSFLAG_RETURN_FILE_MEM, NULL, allowCopyFiles, gamedir);
}
...
======================
*/
bool idFileSystemLocal::FileExists(const char * fileName) {
	idFileScoped file(OpenFileRead(fileName));
