	...
	while ( src.ReadToken( &token ) ) {
		...
		if ( token == "struct" ) {
			if ( src.CheckTokenString( "VS_IN" ) ) {
				...
				var.type = "vec4";
				var.nameCg = "position";
				var.nameGLSL = "gl_Position";
				var.declareInOut = false;
				varsOut.Append( var );
				var.type = "float";
				var.nameCg = "clip0";
				var.nameGLSL = "gl_ClipDistance[0]";
				var.declareInOut = false;
				varsOut.Append( var );
				var.type = "float";
				var.nameCg = "clip1";
				var.nameGLSL = "gl_ClipDistance[1]";
				var.declareInOut = false;
				varsOut.Append( var );
				var.type = "float";
				var.nameCg = "clip2";
				var.nameGLSL = "gl_ClipDistance[2]";
				var.declareInOut = false;
				varsOut.Append( var );
				var.type = "float";
				var.nameCg = "clip3";
				var.nameGLSL = "gl_ClipDistance[3]";
				var.declareInOut = false;
				varsOut.Append( var );
				var.type = "float";
				var.nameCg = "clip4";
				var.nameGLSL = "gl_ClipDistance[4]";
				var.declareInOut = false;
				varsOut.Append( var );
				var.type = "float";
				var.nameCg = "clip5";
				var.nameGLSL = "gl_ClipDistance[5]";
				var.declareInOut = false;
				varsOut.Append( var );
				continue;
				...
				for ( int i = 0; i < varsOut.Num(); i++ ) {
					if ( varsOut[i].declareInOut ) {
						program += va("layout(location = %d) out ", varsOut[i].bind) + varsOut[i].type + " " + varsOut[i].nameGLSL + ";\n";
					}
				}
...
void idRenderProgManager::LoadGLSLProgram( const int programIndex, const int vertexShaderIndex, const int fragmentShaderIndex ) {
	...
	if ( program ) {
		...
		glBindFragDataLocation( program, 0, "d3_FragColor" );
