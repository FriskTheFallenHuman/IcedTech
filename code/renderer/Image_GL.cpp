...
========================
*/
void idImage::ReadImage(int x, int y, int width, int height, int slice, byte *buffer) {
	Bind();

	if (opts.textureType == TT_CUBIC)
	{
		glGetTexImage(GL_TEXTURE_CUBE_MAP_POSITIVE_X + slice, 0, GL_RGBA, GL_UNSIGNED_BYTE, buffer);
	}
	...
	{
		common->FatalError("idImage::ReadImage: Unsupported texture type!");
	}
