...
bool idRenderModelStatic::ConvertDAEToModelSurfaces(const ColladaParser* dae)
{
	...
	for (objectNum = 0; objectNum < dae->mNodeLibrary.Num(); objectNum++)
	{
		...
		bool normalsParsed = false; // (*mesh)->mNormals.Num() > 0; //mesh->normalsParsed;
		...
		*/
		...
		for (j = 0; j < tri->numVerts; j++)
		{
			...
			*(unsigned*)tri->verts[j].color = *(unsigned*)mv->color;
			...
			if ((*mesh)->mTexCoords.Num() > 0)
			{
				...
				tri->verts[j].st.x = u * textureCos + v * textureSin;
				tri->verts[j].st.y = u * -textureSin + v * textureCos;
			}
		}
		...
		idStr materialName = name;
		materialName.StripFileExtension();
		materialName += va("_%d", shaderSurfaceId);

		surf.shader = declManager->FindMaterial(materialName);
		surf.id = shaderSurfaceId - 1;
		surf.geometry = tri;
